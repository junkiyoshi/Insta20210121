...
void ofApp::setup() {
	...
	this->span = 5;
	this->number_of_columns = fbo.getWidth() / this->span;
	ofPixels pixels;
	...
	for (int y = 0; y < fbo.getHeight(); y += span) {
		...
		for (int x = 0; x < fbo.getWidth(); x += span) {
			...
			this->location_list.push_back(glm::vec3(x - ofGetWidth() * 0.5, ofGetHeight() - y - ofGetWidth() * 0.25, 0));
...
void ofApp::update() {
	...
	for (int z = -150; z <= 150; z += 150) {
		...
		for (int i = 0; i < tmp_location_list.size(); i++) {
			...
			auto noise_value = ofNoise(noise_seed, location.x * len_param, location.y * len_param + ofGetFrameNum() * frame_param) * this->param_list[i];
			...
			if (tmp_index < 0 || ofNoise(noise_seed, tmp_location_list[tmp_index].x * len_param, tmp_location_list[tmp_index].y * len_param + ofGetFrameNum() * frame_param) * this->param_list[tmp_index] < threshold) {
			...
			if (tmp_index > tmp_location_list.size() || ofNoise(noise_seed, tmp_location_list[tmp_index].x * len_param, tmp_location_list[tmp_index].y * len_param + ofGetFrameNum() * frame_param) * this->param_list[tmp_index] < threshold) {
			...
			tmp_index = i + 1;
			if (tmp_index % this->number_of_columns == 0 || ofNoise(noise_seed, tmp_location_list[tmp_index].x * len_param, tmp_location_list[tmp_index].y * len_param + ofGetFrameNum() * frame_param) * this->param_list[tmp_index] < threshold) {
			...
			tmp_index = i - 1;
			if (i % this->number_of_columns == 0 || ofNoise(noise_seed, tmp_location_list[tmp_index].x * len_param, tmp_location_list[tmp_index].y * len_param + ofGetFrameNum() * frame_param) * this->param_list[tmp_index] < threshold) {
